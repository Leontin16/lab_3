...
## Особенности сборки и запуска первой тестовой задачи (`p1_first_test`)

Данная подзадача реализует численный метод баланса для стационарного уравнения теплопроводности с разрывными коэффициентами на базе фреймворка **MPI** (Message Passing Interface). Программа автоматически строит таблицу сходимости, рассчитывает логарифмические показатели погрешности `-lg(eps1)` и замеряет общее время выполнения вычислений на параллельных процессах.

### Предварительные требования (Prerequisites)
Для корректной сборки подзадачи в системе должен быть установлен Microsoft MPI (для Windows) или MPICH/OpenMPI (для Linux/WSL).
* На Windows убедитесь, что пути к `mpiexec.exe` и `mpi.h` зарегистрированы в системных переменных среды (`PATH`).
...
# Запуск на x процессах:
```powershell
mpiexec -n x .\build\core\p1_first_test\Release\solver.exe
```
